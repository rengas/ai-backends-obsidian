    ...
    this.registerEvent(
      this.app.workspace.on("editor-menu", (menu, editor, view) => {
        ...
        if (selection.length > 0) {
          menu.addItem((item) => {
            item.setTitle("AI Backends").setIcon("wand").setSection("selection");
            const actionsMenu = item.setSubmenu();
            actionsMenu.addItem((subItem) => {
              subItem.setTitle("Improve description").setIcon("file-text").onClick(async () => {
                await this.improveDescription(editor, selection);
              });
            });
            actionsMenu.addItem((subItem) => {
              subItem.setTitle("Summarise writing").setIcon("document-text").onClick(async () => {
            ...
            actionsMenu.addItem((tItem) => {
              tItem.setTitle("Translate to ...").setIcon("languages");
              const langMenu = tItem.setSubmenu();
              const languages2 = [
    ...
    this.addCommand({
      ...
      editorCallback: async (editor) => {
        ...
        if (selection.length > 0) {
          ...
          new import_obsidian.Notice("Please select some text to brainstorm on");
        }
      }
    });
    this.addCommand({
      ...
      editorCallback: async (editor) => {
        ...
        if (selection.length > 0) {
          ...
          new import_obsidian.Notice("Please select some text to shorten");
        }
      }
    });
      ...
      this.addCommand({
        ...
        editorCallback: async (editor) => {
          ...
          if (selection.length > 0) {
            ...
            new import_obsidian.Notice("Please select some text to change tone");
          }
        }
      });
    }
...
**Summary:**
 ...
 ${result.summary}`, endOfDocument2, endOfDocument2);
  ...
  async rewriteText(editor, text, instruction, headerLabel) {
    if (!this.config || !this.config.rewrite) {
      new import_obsidian.Notice("Please configure the rewrite settings in the YAML file first");
      return;
    }
    if (!this.settings.apiUrl) {
      new import_obsidian.Notice("Please set the API URL in settings");
    ...
    try {
      ...
      const contentType = response.headers.get("content-type") || "";
      const isStreaming = this.config.rewrite.stream && (contentType.includes("text/event-stream") || contentType.includes("application/x-ndjson") || response.body);
      if (isStreaming && response.body) {
        await this.handleStreamingResponse(
          response,
          editor,
...
${output}`, endOfDocument, endOfDocument);
        new import_obsidian.Notice("Action applied successfully");
      }
    } catch (error) {
      console.error("Error applying rewrite:", error);
      new import_obsidian.Notice("Error applying action. Please check your API settings.");
    }
  }
  async improveDescription(editor, text) {
  ...
  async improveWriting(editor, text) {
  ...
  async fixSpellingGrammar(editor, text) {
  ...
  async changeTone(editor, text, tone) {
