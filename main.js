...
var AIPluginSettingTab = class extends import_obsidian10.PluginSettingTab {
  ...
  displayGeneralSettings() {
    ...
    new import_obsidian10.Setting(container).setName("API URL").setDesc("Enter the base URL for the AI API (e.g., http://localhost:3000)").addText((text) => text.setPlaceholder("http://localhost:3000").setValue(this.plugin.settings.apiUrl).onChange(async (value) => {
      this.plugin.settings.apiUrl = value;
      await this.plugin.saveSettings();
    }));
    new import_obsidian10.Setting(container).setName("API Key").setDesc("Enter your API key").addText((text) => text.setPlaceholder("Enter your API key").setValue(this.plugin.settings.apiKey || "").onChange(async (value) => {
      this.plugin.settings.apiKey = value;
      await this.plugin.saveSettings();
    }));
    container.createEl("h3", { text: "Global Defaults" });
    new import_obsidian10.Setting(container).setName("Default Provider").setDesc("Default AI provider for all operations").addDropdown((dropdown) => dropdown.addOption("ollama", "Ollama").addOption("openai", "OpenAI").addOption("anthropic", "Anthropic").setValue(this.plugin.settings.defaultProvider || "ollama").onChange(async (value) => {
      this.plugin.settings.defaultProvider = value;
      await this.plugin.saveSettings();
    }));
    new import_obsidian10.Setting(container).setName("Default Model").setDesc("Default AI model for all operations").addText((text) => text.setPlaceholder("gemma3:4b").setValue(this.plugin.settings.defaultModel || "").onChange(async (value) => {
      this.plugin.settings.defaultModel = value;
      await this.plugin.saveSettings();
    }));
    new import_obsidian10.Setting(container).setName("Default Temperature").setDesc("Default temperature (0.0 = conservative, 1.0 = creative)").addSlider((slider) => slider.setLimits(0, 1, 0.1).setValue(this.plugin.settings.defaultTemperature || 0.3).setDynamicTooltip().onChange(async (value) => {
      this.plugin.settings.defaultTemperature = value;
      await this.plugin.saveSettings();
    }));
    new import_obsidian10.Setting(container).setName("Default Streaming").setDesc("Enable streaming responses by default").addToggle((toggle) => toggle.setValue(this.plugin.settings.defaultStream || true).onChange(async (value) => {
  ...
  createOperationAccordion(container, name, config, onSave, includeLanguage = false) {
    ...
    new import_obsidian10.Setting(content).setName("Provider").setDesc("AI provider for this operation").addDropdown((dropdown) => dropdown.addOption("ollama", "Ollama").addOption("openai", "OpenAI").addOption("anthropic", "Anthropic").setValue(config.provider).onChange(async (value) => {
      config.provider = value;
      await onSave(config);
    }));
    new import_obsidian10.Setting(content).setName("Model").setDesc("AI model to use").addText((text) => text.setPlaceholder("Model name").setValue(config.model).onChange(async (value) => {
      config.model = value;
      await onSave(config);
    }));
    new import_obsidian10.Setting(content).setName("Temperature").setDesc("Temperature (0.0 = conservative, 1.0 = creative)").addSlider((slider) => slider.setLimits(0, 1, 0.1).setValue(config.temperature).setDynamicTooltip().onChange(async (value) => {
      config.temperature = value;
      await onSave(config);
    }));
    new import_obsidian10.Setting(content).setName("Streaming").setDesc("Enable streaming responses").addToggle((toggle2) => toggle2.setValue(config.stream).onChange(async (value) => {
      config.stream = value;
      await onSave(config);
    }));
    if (name === "Summarize" || name === "Compose") {
      new import_obsidian10.Setting(content).setName("Max Length").setDesc("Maximum length of the output").addText((text) => text.setPlaceholder("100").setValue(config.maxLength.toString()).onChange(async (value) => {
        config.maxLength = parseInt(value) || 100;
        await onSave(config);
      }));
    }
    if (name === "Keywords") {
      new import_obsidian10.Setting(content).setName("Max Keywords").setDesc("Maximum number of keywords to extract").addText((text) => text.setPlaceholder("10").setValue(config.maxKeywords.toString()).onChange(async (value) => {
        config.maxKeywords = parseInt(value) || 10;
        await onSave(config);
      }));
    }
    if (includeLanguage) {
      new import_obsidian10.Setting(content).setName("Default Target Language").setDesc("Default language for translation").addDropdown((dropdown) => {
  ...
  displayAdvancedSettings() {
    ...
    new import_obsidian10.Setting(container).setName("Import from YAML").setDesc("Import configuration from existing YAML file").addButton((button) => button.setButtonText("Import").setCta().onClick(async () => {
    ...
    new import_obsidian10.Setting(container).setName("Export to YAML").setDesc("Export current configuration to YAML format").addButton((button) => button.setButtonText("Export").onClick(async () => {
      try {
    ...
    new import_obsidian10.Setting(container).setName("Reset to Defaults").setDesc("Reset all settings to default values").addButton((button) => button.setButtonText("Reset").setWarning().onClick(async () => {
    ...
    new import_obsidian10.Setting(container).setName("Test API Connection").setDesc("Test the current API configuration").addButton((button) => button.setButtonText("Test").onClick(async () => {
      try {
    ...
    new import_obsidian10.Setting(container).setName("View Current Configuration").setDesc("Show the current configuration in JSON format").addButton((button) => button.setButtonText("View").onClick(() => {
