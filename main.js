...
var AIPlugin = class extends import_obsidian.Plugin {
  ...
  // Store reference to the file watcher
  async onload() {
      ...
      this.app.workspace.on("editor-menu", (menu, editor, view) => {
        ...
        if (selection.length > 0) {
          menu.addItem((item) => {
            ...
            subMenu.addItem((subItem) => {
              subItem.setTitle("Translate").setIcon("languages").onClick(async () => {
    ...
    this.addCommand({
      ...
      editorCallback: async (editor, view) => {
        ...
        if (selection.length > 0) {
          ...
          new import_obsidian.Notice("Please select some text to translate");
        }
      }
    });
  ...
  async handleStreamingResponse(response, editor, headerText, successMessage) {
    ...
    try {
      ...
      new import_obsidian.Notice("Error during streaming: " + streamError.message);
    } finally {
      reader.releaseLock();
    }
    new import_obsidian.Notice(successMessage);
  ...
  async summarizeText(editor, text) {
    ...
    try {
      ...
      if (isStreaming && response.body) {
        ...
        const lastLine2 = editor.lastLine();
        const lastLineContent2 = editor.getLine(lastLine2);
        const endOfDocument2 = { line: lastLine2, ch: lastLineContent2.length };
        editor.replaceRange(`

**Summary:**

 ${result.summary}`, endOfDocument2, endOfDocument2);
  ...
  async translateText(editor, text, customTargetLanguage) {
    if (!this.config || !this.config.translate) {
      new import_obsidian.Notice("Please configure the translate settings in the YAML file first");
      return;
    }
    if (!this.settings.apiUrl) {
      new import_obsidian.Notice("Please set the API URL in settings");
    ...
    if (!targetLanguage) {
      new import_obsidian.Notice("Please specify a target language in the config file or provide one");
      return;
    }
    try {
      const requestBody = {
      ...
      editor.setCursor(endOfDocument);
      const contentType = response.headers.get("content-type") || "";
      const isStreaming = this.config.translate.stream && (contentType.includes("text/event-stream") || contentType.includes("application/x-ndjson") || response.body);
      if (isStreaming && response.body) {
        await this.handleStreamingResponse(
          response,
          editor,
...
${result.translation}`, endOfDocument2, endOfDocument2);
        new import_obsidian.Notice("Text translated successfully");
      }
    } catch (error) {
      console.error("Error translating text:", error);
      new import_obsidian.Notice("Error translating text. Please check your API settings.");
    }
  }
